...
from src.constants import ROOTED_SCHEMAS_DIR, MARKDOWN_DIR, TABLES_SIDEBAR_JS_PATH
...
def generate_markdown() -> None:
    ...
    for schema_path in get_all_schema_path():
        markdown_path = schema_path.replace(ROOTED_SCHEMAS_DIR, MARKDOWN_DIR).replace('.json', '.md')
...
def generate_table_sidebar() -> None:
    ...
    sidebar = []
    for product in sorted(os.listdir(ROOTED_SCHEMAS_DIR)):
        if product == 'PMSI':
            children = list()
            for subproduct in sorted(os.listdir(os.path.join(ROOTED_SCHEMAS_DIR, product))):
...
def generate_product_directory_children(product_directory_rel_path):
    product_directory_abs_path = os.path.join(ROOTED_SCHEMAS_DIR, product_directory_rel_path)
