...
def update_byproduct_repository(byproduct_repository: str,
    ...
    try:
        ...
        update_local_byproduct_repository(byproduct_repository, local_to_byproduct_directories,
                                          local_to_byproduct_files)
        if not local:
            update_remote_byproduct_repository(last_commit_id, byproduct_repository, byproduct_project_id,
                                               automatic_merge)
        return 0
...
def update_remote_byproduct_repository(last_commit_id: str,
    ...
    if not exec_terminal("git status --porcelain"):
        ...
        branch_name = "update-from-schema-snds-{}".format(last_commit_id)
        commit_and_push_modifications(branch_name, last_commit_id)
...
def commit_and_push_modifications(branch_name: str, last_commit_id: str):
    exec_terminal("git checkout -b {}".format(branch_name))
    exec_terminal("git add -A")

    commit_message = "MAJ auto de'schema-snds', commit {}".format(last_commit_id)
