...
def update_all_byproducts(local, work_dir) -> None:
    ...
    update_errors += update_byproduct_repository(
        ...
        local_to_byproduct_directories=[
            ('schemas-md', pjoin('tables', '.schemas'))],
...
def update_byproduct_repository(byproduct_repository: str,
    ...
    :param work_dir: Working directory to get byproducts
    ...
    :param has_pipeline: Do remote repository has pipeline
    """
    ...
    try:
        ...
        if not local:
            update_remote_byproduct_repository(last_commit_id_str, byproduct_repository, byproduct_project_id,
                                               automatic_merge, work_dir, has_pipeline=has_pipeline)
        return 0
...
def update_remote_byproduct_repository(last_commit_id_str: str,
    ...
    if not exec_terminal("git status --porcelain"):
        ...
        if automatic_merge:
            merge(byproduct_project_id, merge_request_iid, has_pipeline)
    os.chdir(current_dir)
...
def commit_and_push_modifications(branch_name: str, last_commit_id_str: str):
    logging.info("Création d'une nouvelle branche '{}".format(branch_name))
    exec_terminal("git checkout -b {}".format(branch_name))
    exec_terminal("git add -A")

    commit_message = "MAJ auto de'schema-snds', commit {}".format(last_commit_id_str)
...
def merge(project_id: int, merge_request_iid: int, has_pipeline=True) -> None:
    logging.info('Approve merge request.')
    sleep(1)
    r = requests.post(
    ...
    logging.info('Automatically merge when pipeline succeeds.')
    sleep(1)
    r = requests.put(
        GITLAB_COM_API_V4 + "/projects/{}/merge_requests/{}/merge".format(project_id, merge_request_iid),
        params={
            'merge_when_pipeline_succeeds': 'true' if has_pipeline else 'false',
            'private_token': GITLAB_TOKEN
        }
    )
    check_response_code(r)
...
def check_response_code(response: requests.Response) -> None:
    ...
    if not str(response.status_code).startswith("20"):
        ...
        raise RuntimeError("wrong status_code : {}".format(response.status_code))
    else:
